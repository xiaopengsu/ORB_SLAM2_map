...
void System::SaveKeyFrameTrajectoryTUM(const string &filename)
{
    ...
    void System::SaveReLocalFrameTrajectoryTUM(const string &filename,const double &timestamp,const cv::Mat Tcw,float scale)
    {
        cout << endl << "Saving relocal frame trajectory to " << filename << " ..." << endl;
        ...
        tcw_sx= Tcw.at<float>(0, 3) * scale;
        tcw_sy= Tcw.at<float>(1, 3) * scale;
        tcw_sz=Tcw.at<float>(2, 3) * scale;
        ...
        cv::Mat Rcw = Tcw.rowRange(0,3).colRange(0,3);// cv::Mat Tcwr=Tcw.clone();cv::Mat Rcw = Tcwr(cv::Rect(0,0,3,3));
            ...
            f << setprecision(6) << timestamp<< setprecision(7) << " " << tcw_sx << " " << tcw_sy<< " " << tcw_sz
              << " " << q[0] << " " << q[1] << " " << q[2] << " " << q[3] << endl;

        f.close();
        cout << endl << "relocal trajectory saved!" << endl;
    }


    void System::SaveEveryFrameTrajectoryTUMCount(const string &filename,const int &count,const double &timestamp,const cv::Mat Tcw, float scale)
    {
        cout << endl << "Saving relocal frame trajectory to " << filename << " ..." << endl;
        ...
        cv::Mat Rcw = Tcw.rowRange(0,3).colRange(0,3);// cv::Mat Tcwr=Tcw.clone();cv::Mat Rcw = Tcwr(cv::Rect(0,0,3,3));
        vector<float> q = Converter::toQuaternion(Rcw);
        f <<count<<" "<< setprecision(6) << timestamp<< setprecision(7) << " " << tcw_sx << " " << tcw_sy<< " " << tcw_sz
          << " " << q[0] << " " << q[1] << " " << q[2] << " " << q[3] << endl;
